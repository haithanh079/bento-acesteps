...
ACE-Step can be deployed as a serverless worker on RunPod for scalable, on-demand music generation.
...
1. **Initialize RunPod Project** (optional):
   ```bash
   runpod project new --init
   ```

2. **Configure Deployment**:
   - Edit `runpod.toml` to set GPU requirements, container disk size, and environment variables
   - Recommended GPU: NVIDIA A100 or RTX 4090
   - Recommended container disk: 50GB+ for models
...
job = endpoint.run({
    "input": {
        ...
        "checkpoint_path": "",  # Optional: auto-downloads if empty
        "bf16": True,
        "torch_compile": False,
        "cpu_offload": False,
...
{
    "status": "success",
    "output_url": "https://s3-presigned-url-to-audio.wav",
    "output_path": "/tmp/acestep_jobid_abc123.wav",
...
Then test with a sample request:

```bash
curl -X POST http://localhost:8000/runsync \
  -H "Content-Type: application/json" \
  -d '{"input": {"prompt": "pop music", "audio_duration": 30.0}}'
```

**S3 Configuration (Optional):**

To enable automatic S3 uploads, set these environment variables in `runpod.toml`:
...
BUCKET_PREFIX = "acestep-outputs"
```

If S3 is not configured, the handler will return the local file path instead of a presigned URL.
